...
class TutorManager:
    ...
    def _permission_config(
        ...
        entries = {":minimal": "read", **{str(path): "read" for path in paths}}
        entries.update({str(path): "deny" for path in denied_paths or []})
        body = ",".join(
            f"{_toml_string(key)}={_toml_string(value)}" for key, value in entries.items()
        )
