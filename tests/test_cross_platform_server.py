...
class CrossPlatformServerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "ui").mkdir()
        (self.root / "windows" / "reader").mkdir(parents=True)
        payload = b"%PDF-1.1\n%%EOF\n"
        ...
        (self.root / "ui" / "index.html").write_text("<h1>Fixture</h1>", encoding="utf-8")
        (self.root / "ui" / "styles.css").write_text("", encoding="utf-8")
        (self.root / "ui" / "app.js").write_text("", encoding="utf-8")
        (self.root / "windows" / "reader" / "ping.txt").write_text("pong", encoding="utf-8")
        self.database = self.root / "reader-state.sqlite3"
    ...
    def test_health_identifies_exact_library_and_protocol(self) -> None:
    ...
    def test_state_round_trip_export_and_import(self) -> None:
        ...
        self.server.state_store.delete_value("localStorage", "cs-library:test")
        ...
        self.assertEqual(
            self.server.state_store.snapshot("localStorage")["cs-library:test"],
            '{"page":4}',
        )

    def test_state_api_rejects_missing_token(self) -> None:
        with self.assertRaises(urllib.error.HTTPError) as caught:
            self.request("/api/state/snapshot")
        self.assertEqual(caught.exception.code, 403)
        caught.exception.close()

    def test_windows_reader_route_is_scoped(self) -> None:
        with urllib.request.urlopen(self.base + "/windows-reader/ping.txt", timeout=3) as response:
            self.assertEqual(response.read(), b"pong")
        with self.assertRaises(urllib.error.HTTPError) as caught:
            urllib.request.urlopen(self.base + "/windows-reader/%2E%2E/CATALOG.md", timeout=3)
